...
function parseNaturalLanguageQuery(text) {
  ...
  // 1. Extract 4-digit Year (1950 - 2030)
  const yearMatch = raw.match(/\b(19[5-9]\d|20[0-3]\d)\b/);
  if (yearMatch) {
    year = yearMatch[1];
  }

  // 2. Extract Make (case-insensitive match against COMMON_MAKES)
  ...
  // 3. Extract Model & Trim if Make was found
  if (make) {
    ...
    // Next token is likely model (e.g., Civic, F-150, Silverado, M3, Camry)
    if (tokens.length > 0 && !/^(for|with|in|and|the|a|an|parts?|for|of)$/i.test(tokens[0])) {
      model = tokens[0];

      // Second token could be Trim (EX, LX, LT, Sport, Limited, GT, Base)
  ...
  // 4. Extract Category / Keyword (e.g. brake pads, oil filter, wipers, rotors, spark plugs)
  const keywordsList = [
...
export async function action({ request }) {
  ...
  if (!shop) {
    return json({ error: "Could not resolve shop for AI Voice search request", success: false }, { status: 400 });
  }
  ...
  if (!queryText.trim()) {
    return json({ error: "Query parameter is required", success: false }, { status: 400 });
  }
